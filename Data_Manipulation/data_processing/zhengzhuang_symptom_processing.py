...
def main():
    ...
    # 写入文件头
    with open(os.path.join(local_url, 'zhengzhuang_new_node.csv'), 'w') as f:
        f.write("title,lable" + '\n')
    with open(os.path.join(local_url, 'zhengzhuang_symptom_relation.csv'), 'w') as f:
        f.write("entity1,relation,entity2" + '\n')
    with open(os.path.join(local_url, 'zhengzhuang_symptom_relation_newnode.csv'), 'w') as f:
        f.write("entity,relation,NewNode" + '\n')

    # 读取entityRelation.json并将数据追加到另外三个文件中
    with open(os.path.join(local_url, 'zhengzhuang_symptom_corrected.json'), 'r') as fer:
        with open(os.path.join(local_url, 'zhengzhuang_new_node.csv'), 'a') as fnn:
            with open(os.path.join(local_url, 'zhengzhuang_symptom_relation.csv'), 'a') as fwr:
                with open(os.path.join(local_url, 'zhengzhuang_symptom_relation_newnode.csv'), 'a') as fwrn:
                    newNodeList = list()
