...
class mongo:
    ...
    def insertDoctor(self, doctordict):
    ...
    def getMaxId(self, id_):
        query = {'doctorId': {"$regex": "^" + str(id_)}}
        x = self.collection(settings['collection_name']).find(query).sort([('doctorId', -1)]).limit(1)
