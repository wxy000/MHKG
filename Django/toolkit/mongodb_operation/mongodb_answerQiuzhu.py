...
settings = {
    ...
    "port": 27017,
    "db_name": "doctor",
    "collection_name": "answerQiuzhu"
}


class mongo_answer_qiuzhu:
    def __init__(self):
        try:
    ...
    def insertAnswerQiuzhu(self, doctordict):
    ...
    def getMaxId(self, id_):
        query = {'answerQiuzhuId': {"$regex": "^" + str(id_)}}
        x = self.collection(settings['collection_name']).find(query).sort([('answerQiuzhuId', -1)]).limit(1)
    ...
    #     return x
    #
    def getAnswerQiuzhuByQuery(self, query):
